...
export async function writeGroundedDocument(query, profileData, evidencePassages, mcqResponses = [], uploadedDocsContent = []) {
  ...
  try {
    ...
    console.log('[WRITER] Réponse reçue, longueur:', response?.length || 0);
    ...
    try {
      ...
      console.log('[WRITER] Parsing direct échoué, recherche JSON dans la réponse...');
      ...
      if (jsonMatch) {
        try {
          ...
          console.log('[WRITER] Parsing JSON extrait échoué, tentative de nettoyage...');
          // Nettoyer le JSON (enlever les caractères problématiques)
          const cleaned = jsonMatch[0]
            .replace(/[\x00-\x1F\x7F]/g, ' ') // Enlever les caractères de contrôle
            .replace(/,\s*}/g, '}') // Enlever les virgules trailing
            .replace(/,\s*]/g, ']');
          documentData = JSON.parse(cleaned);
        }
      } else {
        // Si pas de JSON trouvé, créer un document à partir du texte brut
        console.log('[WRITER] Aucun JSON trouvé, création document depuis texte brut...');
        documentData = {
          title: `Document sur ${query.substring(0, 50)}`,
          sections: [{
            title: 'Contenu',
            content: response || 'Contenu non disponible'
          }]
        };
      }
    }
...
FORMAT JSON STRICT:
{
  ...
  try {
    ...
    if (jsonMatch) {
      ...
      console.log('[WRITER] Plan généré:', plan.sections?.length || 0, 'sections');
    } else {
      throw new Error('Pas de JSON dans la réponse du plan');
    }
...
Réponds UNIQUEMENT avec le contenu de la section (pas de titre, pas de JSON).`;
    ...
    try {
      let sectionContent = await generateText(sectionPrompt, { 
        maxTokens: 6000, // Augmenté pour permettre des sections plus longues
        temperature: 0.3 
      });
      
      // Vérifier si la section est trop courte et régénérer si nécessaire
      let wordCount = sectionContent.trim().split(/\s+/).length;
      let attempts = 0;
      
      while (wordCount < wordsPerSection * 0.5 && attempts < 2) {
        console.log(`[WRITER] Section trop courte (${wordCount} mots), régénération...`);
        attempts++;
        
        const retryPrompt = `La section précédente était trop courte. DÉVELOPPE BEAUCOUP PLUS.

SUJET: "${query}"
SECTION: "${sectionTitle}"

Tu DOIS écrire MINIMUM ${wordsPerSection} MOTS. Développe avec :
- Des paragraphes longs et détaillés
- Des exemples concrets
- Des explications approfondies
- Des nuances et analyses

Écris le contenu de la section MAINTENANT (texte uniquement, pas de titre):`;

        sectionContent = await generateText(retryPrompt, { maxTokens: 6000, temperature: 0.4 });
      ...
      console.log(`[WRITER] Section générée: ${wordCount} mots, ${sectionContent.length} caractères`);
    } catch (e) {
      console.error(`[WRITER] Erreur section ${sectionTitle}:`, e.message);
      allSections.push({
        title: sectionTitle,
        content: `Cette section traite de ${sectionTitle.toLowerCase()} dans le contexte de ${query}. Une erreur s'est produite lors de la génération détaillée.`
      });
    }
  }
  ...
  const totalChars = allSections.reduce((sum, s) => sum + s.content.length, 0);
  const estimatedPages = Math.round(totalChars / 3500);
  
  console.log(`[WRITER] Document long généré: ${totalWords} mots, ${totalChars} caractères, ~${estimatedPages} pages`);
